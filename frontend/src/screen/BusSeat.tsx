...
const saveBookingDetails = async () => {
  ...
  if (!token) {
    await Swal.fire({
      ...
      text: 'Please log in to book your seat.',
      icon: 'warning',
  ...
  try {
    ...
    const orderRes = await axios.post(
      "https://bus-booking-microservice.onrender.com/api/payment/createorder",
      { amount: data.price }
    );

    const options = {
      key: "rzp_test_JM6LShfFVZeJ40", // Replace with your Razorpay test/live key
      ...
      handler: async function () {
        try {
          await axios.post(
            "https://bus-booking-microservice.onrender.com/api/bookseat",
            data,
            {
              headers: {
    ...
    Swal.fire({
      title: "Error!",
      text: "Something went wrong during payment or booking.",
      icon: "error",
    });
    setLoading(false);
  }
